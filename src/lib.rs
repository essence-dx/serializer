...
///
/// The implementation now lives under [`machine`], but the public `zero`
/// namespace remains available for existing callers and specification tests.
pub mod zero {
    /// Header validation exports for callers that import `zero::header::HeaderError`.
    pub mod header {
        pub use crate::machine::header::HeaderError;
    }

    pub use crate::machine::compress::{StreamCompressor, StreamDecompressor};
    pub use crate::machine::header::{
        FLAG_HAS_HEAP, FLAG_HAS_INTERN, FLAG_HAS_LENGTH_TABLE, FLAG_LITTLE_ENDIAN, HeaderError,
    };
    pub use crate::machine::{
        CompressionLevel, DxCompressed, DxFormat, DxMachineBuilder as DxZeroBuilder,
        DxMachineHeader as DxZeroHeader, DxMachineSlot as DxZeroSlot, HEAP_MARKER, INLINE_MARKER,
