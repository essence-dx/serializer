...
pub enum FooterError {
    ...
    /// Invalid magic bytes
    InvalidMagic {
        /// Expected footer magic bytes.
        expected: [u8; 4],
        /// Magic bytes found in the payload.
        found: [u8; 4],
    },
    ...
    /// Checksum mismatch
    ChecksumMismatch {
        /// Footer checksum read from the payload.
        expected: u16,
        /// Checksum computed from the data section.
        computed: u16,
    },
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::BufferTooSmall => {
                write!(
                    f,
                    "Buffer too small to contain footer (need {FOOTER_SIZE} bytes)"
                )
            }
            ...
            Self::UnsupportedVersion { supported, found } => write!(
                f,
                "Unsupported footer version: this implementation supports v{supported:02X}, found v{found:02X}"
            ),
            Self::ChecksumMismatch { expected, computed } => write!(
                f,
                "Checksum mismatch: expected 0x{expected:04X}, computed 0x{computed:04X}"
            ),
        }
    }
}
