...
impl DxMachineSlot {
    ...
    pub fn inline_from_bytes(bytes: &[u8]) -> Result<Self, SlotError> {
        ...
        slot.data[1..=len].copy_from_slice(bytes);
    ...
    pub fn write_inline(&mut self, bytes: &[u8]) -> Result<(), SlotError> {
        ...
        self.data[1..=len].copy_from_slice(bytes);
        self.data[15] = INLINE_MARKER;
...
pub enum SlotError {
    ...
    DataTooLarge {
        /// Attempted inline payload size in bytes.
        size: usize,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::DataTooLarge { size, max } => {
                write!(f, "Data size {size} exceeds max inline size {max}")
            }
            Self::InvalidMarker { found } => {
                write!(f, "Invalid slot marker byte: 0x{found:02X}")
            }
        }
    }
}
