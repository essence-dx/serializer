...
impl DxMachineHeader {
    ...
    /// Set heap flag
    #[inline]
    pub const fn set_has_heap(&mut self, value: bool) {
        if value {
    ...
    #[inline]
    pub const fn set_has_intern_table(&mut self, value: bool) {
        if value {
    ...
    #[inline]
    pub const fn set_has_length_table(&mut self, value: bool) {
        if value {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedVersion { supported, found } => write!(
                f,
                "Unsupported format version: this implementation supports v{supported:02X}, found v{found:02X}"
            ),
