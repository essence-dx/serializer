...
impl<'a> DxMachineBuilder<'a> {
    ...
    pub fn write_fixed<T: Copy>(&mut self, offset: usize, value: T) {
        // SAFETY: `value` is a live `T` reference and this creates a read-only
        // byte view exactly `size_of::<T>()` long for immediate copying.
        let bytes = unsafe {
            std::slice::from_raw_parts((&raw const value).cast::<u8>(), std::mem::size_of::<T>())
        };
    ...
    pub fn write_array<T: Copy>(&mut self, slot_offset: usize, values: &[T]) {
        // SAFETY: `values` is a live slice and this creates a read-only byte
        // view exactly `size_of_val(values)` long for immediate copying.
        let bytes = unsafe {
