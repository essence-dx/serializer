...
/// # Safety
///
/// The byte slice must contain a valid archived representation of `T` produced
/// by a trusted RKYV serializer. The returned reference is tied to `bytes`, so
/// callers must keep the byte slice alive and immutable for the full lifetime
