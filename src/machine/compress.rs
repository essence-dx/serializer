...
impl CompressionLevel {
    ...
    pub const fn to_zstd_level(self) -> i32 {
        match self {
            Self::Fast => 1,    // Fastest
            Self::Default => 3, // Balanced (default)
            Self::High => 19,   // Maximum compression
        }
    }
}
...
impl DxCompressed {
    ...
    pub fn ratio(&self) -> f64 {
        ...
        self.compressed.len() as f64 / f64::from(self.original_size)
    }
    ...
    #[must_use]
    pub const fn from_compressed(compressed: Vec<u8>, original_size: u32) -> Self {
        Self {
            compressed,
            original_size,
            decompressed: None,
    ...
    while i < input.len() {
        ...
        if run_len >= 4 {
            ...
            if lit_len > 0 {
                ...
                let mut offset = lit_start;
                while remaining > 0 {
                    let chunk = remaining.min(0xFE);
...
impl StreamDecompressor {
    ...
    #[must_use]
    pub const fn new(chunks: Vec<DxCompressed>) -> Self {
        Self {
            chunks,
            current_chunk: 0,
            current_offset: 0,
    ...
    pub fn decompress_all(&mut self) -> Result<Vec<u8>> {
        let total_size: usize = self.chunks.iter().map(DxCompressed::original_size).sum();
...
pub fn decompress_lz4(data: &[u8]) -> Result<Vec<u8>> {
    ...
    lz4_flex::decompress_size_prepended(data)
        .map_err(|e| DxMachineError::DecompressionFailed(format!("LZ4 decompression failed: {e}")))
}
