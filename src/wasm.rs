...
impl SerializerConfig {
    ...
    pub const fn set_indent_size(&mut self, size: usize) {
    ...
    pub const fn set_preserve_comments(&mut self, preserve: bool) {
        self.preserve_comments = preserve;
    }

    /// Set whether to use smart quoting
    #[cfg_attr(feature = "wasm", wasm_bindgen(js_name = setSmartQuoting))]
    pub const fn set_smart_quoting(&mut self, smart: bool) {
        ...
        if in_string {
            if let Some((line, col)) = string_start {
                return ValidationResult::invalid(
                    ...
                    line,
                    col,
                    format!("Add a closing '{string_char}' to complete the string"),
                );
            }
        }
