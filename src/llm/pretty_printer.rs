...
pub enum PrettyPrintError {
    ...
    RoundTripFailed {
        /// Human-readable round-trip mismatch.
        msg: String,
    },
}
    ...
    fn check_round_trip(
        ...
        for (key, value) in &original.context {
            if let Some(parsed_value) = parsed.context.get(key) {
                if !values_equal(value, parsed_value) {
                    return Err(PrettyPrintError::RoundTripFailed {
                        msg: format!(
                            "Context value mismatch for key '{key}': original={value:?}, parsed={parsed_value:?}"
                        ),
                    });
                }
            } else {
                return Err(PrettyPrintError::RoundTripFailed {
                    msg: format!("Context key '{key}' missing in parsed document"),
                });
            }
        }
        ...
        for (id, section) in &original.sections {
            if let Some(parsed_section) = parsed.sections.get(id) {
                    ...
                    .enumerate()
                {
                        ...
                        orig_row.iter().zip(parsed_row.iter()).enumerate()
                    {
                        if !values_equal(orig_val, parsed_val) {
                            return Err(PrettyPrintError::RoundTripFailed {
                                msg: format!(
                                    "Value mismatch in section '{id}' row {row_idx} col {col_idx}: original={orig_val:?}, parsed={parsed_val:?}"
                                ),
                            });
                        }
                    }
                ...
                return Err(PrettyPrintError::RoundTripFailed {
                    msg: format!("Section '{id}' missing in parsed document"),
                });
            }
        }
