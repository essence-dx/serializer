...
pub enum ParseError {
    ...
    UnexpectedChar {
        /// Character that caused parsing to fail.
        ch: char,
        /// Byte position where the character was found.
        pos: usize,
    },
    ...
    InvalidValue {
        /// Original value fragment that could not be parsed.
        value: String,
    },
    ...
    SchemaMismatch {
        /// Number of columns declared by the table schema.
        expected: usize,
        /// Number of values parsed from the row.
        got: usize,
    },
    ...
    Utf8Error {
        /// Byte offset reported by UTF-8 validation.
        offset: usize,
    },
    ...
    UnclosedBracket {
        /// Byte position where the unmatched bracket was opened.
        pos: usize,
    },
    ...
    UnclosedParen {
        /// Byte position where the unmatched parenthesis was opened.
        pos: usize,
    },
    ...
    MissingValue {
        /// Byte position of the incomplete assignment.
        pos: usize,
    },
    ...
    InvalidTable {
        /// Human-readable table parse failure.
        msg: String,
    },
}
...
impl<'a> DsrParser<'a> {
    ...
    fn section_id_for_name(&self, doc: &DxDocument, name: &str) -> char {
        if let Some((section_id, _)) = doc
            .section_names
            .iter()
        ...
        for ch in name.chars().filter(char::is_ascii_alphanumeric) {
            if !doc.sections.contains_key(&ch) {
                return ch;
            }
        }
    ...
    fn parse_statement(&mut self, doc: &mut DxDocument) -> Result<(), ParseError> {
        ...
        let mut parsed_section_id = None;
        ...
        match ch {
            ...
            Some(':') => {
                ...
                if !is_leaf && next == Some('(') {
                    ...
                    doc.section_names.insert(section_id, name.clone());
                    parsed_section_id = Some(section_id);
                } else if !is_leaf && next.is_some_and(|c| c.is_ascii_digit()) {
        ...
        if doc.context.contains_key(&name) {
            ...
            if let Some(section_id) = parsed_section_id {
    ...
    fn parse_until_char(&mut self, target: char) -> Result<String, ParseError> {
        ...
        while self.pos < self.input.len() {
            ...
            } else if !in_quotes && ch == target && depth == 0 {
                break;
            } else {
                value.push(ch);
                self.advance();
...
mod tests {
    ...
    #[test]
    fn test_parse_full_line_comments() {
        let input = r#"
# DX ecosystem config
project.name="dx-www"

// Style and icon tools live in the same extensionless dx file.
tooling.dx_style.mode="generated-css"
tooling.icons.generated_dir="components/icons"
"#;
        let doc = LlmParser::parse(input).unwrap();

        assert_eq!(doc.context.len(), 3);
        assert_eq!(
            doc.context.get("project.name").unwrap().as_str(),
            Some("dx-www")
        );
        assert_eq!(
            doc.context.get("tooling.dx_style.mode").unwrap().as_str(),
            Some("generated-css")
        );
        assert_eq!(
            doc.context
                .get("tooling.icons.generated_dir")
                .unwrap()
                .as_str(),
    ...
    #[test]
    fn test_parse_wrapped_dataframe_tables_with_same_initial() {
        let input = r#"
...
packages[id version source surfaces](
"shadcn/ui/button" 0.1.0 forge "button docs"
)
"#;
        let doc = LlmParser::parse(input).unwrap();
        ...
        assert!(
            doc.section_names
                .values()
                .any(|name| name == "protected_crates")
        );
        assert!(doc.section_names.values().any(|name| name == "paths"));
        assert!(doc.section_names.values().any(|name| name == "packages"));

        let packages_id = doc
            .section_names
            .iter()
            .find_map(|(id, name)| (name == "packages").then_some(*id))
            .unwrap();
        let packages = doc.sections.get(&packages_id).unwrap();
        assert_eq!(packages.schema, vec!["id", "version", "source", "surfaces"]);
        assert_eq!(packages.rows.len(), 1);
        assert_eq!(packages.rows[0][0].as_str(), Some("shadcn/ui/button"));
    }
    ...
    #[test]
    fn test_parse_toml_style_quoted_array_values() {
        let input = r#"routes=["/docs", "/docs/[[...slug]]", "/docs/readiness"]
empty=[]"#;
        let doc = LlmParser::parse(input).unwrap();

        let routes = doc.context.get("routes").unwrap();
        if let DxLlmValue::Arr(items) = routes {
            assert_eq!(items.len(), 3);
            assert_eq!(items[0].as_str(), Some("/docs"));
            assert_eq!(items[1].as_str(), Some("/docs/[[...slug]]"));
            assert_eq!(items[2].as_str(), Some("/docs/readiness"));
        } else {
            panic!("Expected routes array");
        }

        let empty = doc.context.get("empty").unwrap();
        if let DxLlmValue::Arr(items) = empty {
