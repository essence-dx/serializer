...
impl DxDocument {
    ...
    ///
    /// This supports both root keys (`project`) and object paths
    /// (`project.name`) where the root value is an inline object such as
    /// `project(name=dx-devtools version=0.1.0)`.
    #[must_use]
...
impl DxSection {
    ...
    #[must_use]
    pub const fn new(schema: Vec<String>) -> Self {
        Self {
            schema,
            rows: Vec::new(),
        }
...
impl DxLlmValue {
    ...
    /// Try to get as array
    #[must_use]
    pub const fn as_arr(&self) -> Option<&Vec<Self>> {
        match self {
    ...
    #[must_use]
    pub const fn as_obj(&self) -> Option<&IndexMap<String, Self>> {
        match self {
