...
/// Errors returned by the zero-copy machine format reader and writer.
#[derive(Debug, Error)]
pub enum ZeroCopyError {
    /// Magic bytes did not match the zero-copy format marker.
    #[error("Invalid magic number")]
    InvalidMagic,
    /// Format version is newer or older than this implementation supports.
    #[error("Unsupported version: {0}")]
    UnsupportedVersion(u8),
    /// Payload bytes are structurally invalid.
    #[error("Invalid data: {0}")]
    InvalidData(String),
    /// A read attempted to move outside the payload boundary.
...
impl ZeroCopyMachine {
    ...
    /// Decode the machine-format bytes into a document.
    ...
    /// Validate and borrow the encoded bytes through a document view.
    pub fn access(&self) -> Result<ZeroCopyDocument<'_>, ZeroCopyError> {
        ZeroCopyDocument::new(&self.data)
    }
}

/// Borrowed view over a validated zero-copy machine-format payload.
...
impl<'a> ZeroCopyDocument<'a> {
    ...
    /// Decode this borrowed payload into an owned document.
    pub fn to_document(&self) -> Result<DxDocument, ZeroCopyError> {
        ZeroCopyMachine {
