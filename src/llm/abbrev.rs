...
impl AbbrevDict {
    ...
    #[must_use] 
    pub const fn global_mappings(&self) -> &HashMap<&'static str, &'static str> {
        &self.global
    }
...
mod tests {
    ...
    fn test_round_trip_global() {
        ...
        for (&abbrev, &full) in dict.global_mappings() {
            ...
            assert_eq!(
                expanded, full,
                "Round-trip failed for {abbrev} -> {full}"
            );
        }
    }
    ...
    fn test_round_trip_reverse() {
        ...
        for (&abbrev, &_full) in dict.global_mappings() {
            ...
            assert_eq!(
                ...
                "Reverse round-trip failed for {abbrev}: {abbrev} -> {expanded} -> {compressed} -> {re_expanded}"
            );
        }
    }
