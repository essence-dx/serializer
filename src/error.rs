...
impl DxError {
    ...
    pub const fn offset(&self) -> Option<usize> {
        match self {
            ...
            Self::ParseError { location, .. } => Some(location.offset),
            Self::InvalidSyntax { pos, .. } => Some(*pos),
            Self::Utf8Error { offset } => Some(*offset),
            Self::Base62Error { position, .. } => Some(*position),
            Self::DittoNoPrevious(pos) => Some(*pos),
            _ => None,
        }
    }
    ...
    pub const fn is_recoverable(&self) -> bool {
        matches!(
            self,
            Self::UnknownAlias(_) | Self::UnknownAnchor(_) | Self::TypeMismatch { .. }
        )
    }
}
...
impl From<crate::llm::parser::ParseError> for DxError {
    ...
    fn from(err: crate::llm::parser::ParseError) -> Self {
        ...
        match err {
            ParseError::UnexpectedChar { ch, pos } => Self::InvalidSyntax {
                pos,
                msg: format!("Unexpected character '{ch}'"),
            },
            ParseError::UnexpectedEof => Self::UnexpectedEof(0),
            ParseError::InvalidValue { value } => Self::InvalidSyntax {
                pos: 0,
                msg: format!("Invalid value format: {value}"),
            },
            ParseError::SchemaMismatch { expected, got } => Self::SchemaError(format!(
                "Schema mismatch: expected {expected} columns, got {got}"
            )),
            ParseError::Utf8Error { offset } => Self::Utf8Error { offset },
            ParseError::InputTooLarge { size, max } => Self::InputTooLarge { size, max },
            ParseError::UnclosedBracket { pos } => Self::InvalidSyntax {
                pos,
                msg: "Unclosed bracket".to_string(),
            },
            ParseError::UnclosedParen { pos } => Self::InvalidSyntax {
                pos,
                msg: "Unclosed parenthesis".to_string(),
            },
            ParseError::MissingValue { pos } => Self::InvalidSyntax {
                pos,
                msg: "Missing value after '='".to_string(),
            },
            ParseError::InvalidTable { msg } => {
                Self::SchemaError(format!("Invalid table format: {msg}"))
            }
        }
    }
}
...
impl From<crate::llm::convert::ConvertError> for DxError {
    ...
    fn from(err: crate::llm::convert::ConvertError) -> Self {
        ...
        match err {
            ...
            ConvertError::HumanParse(human_err) => Self::ConversionError(human_err.to_string()),
            ConvertError::MachineFormat { msg } => {
                Self::ConversionError(format!("Machine format error: {msg}"))
            }
        }
    }
}
    ...
    fn test_error_from_parse_error_preserves_position() {
        ...
        for (parse_err, expected_pos) in test_cases {
            ...
            assert_eq!(
                dx_err.offset(),
                expected_pos,
                "Position not preserved for error: {dx_err:?}"
            );
        }
    }
