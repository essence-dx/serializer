...
/// ```
pub const fn validate_utf8(bytes: &[u8]) -> Result<&str> {
...
pub fn validate_utf8_detailed(bytes: &[u8]) -> std::result::Result<&str, Utf8ValidationError> {
    ...
    while i < bytes.len() {
        ...
        let seq_len = if byte & 0x80 == 0 {
            ...
            return Err(Utf8ValidationError::new(
                i,
                Some(byte),
                format!("Unexpected continuation byte 0x{byte:02X} at position {i}"),
            ));
        } else {
            // Invalid start byte
            return Err(Utf8ValidationError::new(
                i,
                Some(byte),
                format!("Invalid UTF-8 start byte 0x{byte:02X} at position {i}"),
            ));
        };
        ...
        if seq_len == 2 {
            ...
            let code_point = ((u32::from(byte) & 0x1F) << 6) | (u32::from(bytes[i + 1]) & 0x3F);
            if code_point < 0x80 {
                return Err(Utf8ValidationError::new(
                    i,
                    Some(byte),
                    format!(
                        "Overlong encoding at position {i}: 2-byte sequence for code point U+{code_point:04X}"
                    ),
                ));
            }
        } else if seq_len == 3 {
            // 3-byte sequences must encode values >= 0x800 and not be surrogates
            let code_point = ((u32::from(byte) & 0x0F) << 12)
            ...
            if code_point < 0x800 {
                return Err(Utf8ValidationError::new(
                    ...
                    Some(byte),
                    format!(
                        "Overlong encoding at position {i}: 3-byte sequence for code point U+{code_point:04X}"
                    ),
                ));
            }
            if (0xD800..=0xDFFF).contains(&code_point) {
                return Err(Utf8ValidationError::new(
                    i,
                    Some(byte),
                    format!("Invalid surrogate code point U+{code_point:04X} at position {i}"),
                ));
            }
        } else if seq_len == 4 {
            ...
            if code_point < 0x10000 {
                return Err(Utf8ValidationError::new(
                    ...
                    Some(byte),
                    format!(
                        "Overlong encoding at position {i}: 4-byte sequence for code point U+{code_point:04X}"
                    ),
                ));
            }
            if code_point > 0x10FFFF {
                return Err(Utf8ValidationError::new(
                    ...
                    Some(byte),
                    format!(
                        "Code point U+{code_point:04X} exceeds maximum U+10FFFF at position {i}"
                    ),
                ));
            }
        }
    ...
    // SAFETY: the manual scanner above validates start bytes, continuation
    // bytes, overlong encodings, surrogate ranges, and maximum code point.
...
pub const fn validate_string_input(bytes: &[u8], base_offset: usize) -> Result<&str> {
    match std::str::from_utf8(bytes) {
        Ok(s) => Ok(s),
        Err(e) => Err(DxError::Utf8Error {
