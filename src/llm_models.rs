...
pub enum Provider {
    /// `OpenAI` or Azure `OpenAI` served model.
    OpenAI,
...
impl fmt::Display for Provider {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::OpenAI => write!(f, "OpenAI/Azure"),
...
pub struct TokenAnalysis {
    /// Model name used for this token estimate.
    pub model_name: &'static str,
    /// Provider that serves the model.
    pub provider: Provider,
    /// Published context-window label.
    pub context_window: &'static str,
    /// Estimated token count for the analyzed text.
    pub tokens: usize,
    /// Estimated uncached input cost in USD.
    pub input_cost: f64,
    /// Estimated cached-input cost in USD.
    pub cached_cost: f64,
    /// Estimated output cost in USD for the same token count.
    pub output_cost: f64,
}
