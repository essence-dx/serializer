    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Misaligned { needed, actual } => {
                write!(
                    f,
                    "pointer misaligned: needed {needed} byte alignment, offset is {actual}"
                )
            }
            Self::OffsetOutOfBounds { offset, length } => {
                write!(f, "offset {offset} out of bounds for length {length}")
            }
...
/// ```
#[inline(always)]
pub const fn check_size<T>(slice: &[u8]) -> Result<(), SafetyError> {
...
/// ```
#[inline(always)]
pub const fn check_bounds(offset: usize, size: usize, length: usize) -> Result<(), SafetyError> {
...
pub fn safe_read_slice<T: Copy>(slice: &[u8], count: usize) -> Result<&[T], SafetyError> {
    ...
    Ok(unsafe { core::slice::from_raw_parts(slice.as_ptr().cast::<T>(), count) })
}
...
mod tests {
    ...
    fn test_check_alignment_success() {
        let aligned: [u64; 2] = [0, 0];
        let ptr = aligned.as_ptr().cast::<u8>();
        assert!(check_alignment::<u64>(ptr).is_ok());
