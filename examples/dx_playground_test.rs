...
fn test_dx_json() {
    ...
    if let Ok(json_content) = fs::read_to_string(json_path) {
        ...
        println!(
            "   JSON size: {json_bytes} bytes, ~{json_tokens} tokens"
        );
        ...
        match json_to_dx(&json_content) {
            Ok(dsr) => {
                ...
                println!("   Tokens: ~{dx_tokens} ({token_ratio:.1}× better!)");
                println!("   Byte efficiency: {byte_ratio:.2}× smaller");
                ...
                println!(
                    "   ✅ Token-efficient - {token_ratio:.1}× better than JSON"
                );
                println!("   ✅ Context-friendly - Fit {token_ratio:.1}× more data");
...
fn test_synthetic_data() {
    ...
    println!("Test JSON: {json_bytes} bytes, ~{json_tokens} tokens");
    ...
    match json_to_dx(test_json) {
        Ok(dsr) => {
            ...
            println!("Dx Serializer: {dx_bytes} bytes, ~{dx_tokens} tokens");
            println!(
                "Efficiency: {:.1}× better",
