# DX Serializer vs JSON vs YAML vs TOON — Token Efficiency Benchmarks
...
- **38–65% fewer tokens than JSON** (general data)
- **14–59% fewer tokens than YAML** (general data)
- **36–41% fewer tokens than TOON** (tool calling)

TOON files generated by the official `@toon-format/cli` encoder produce significantly
more tokens than hand-optimized TOON — making DX Compact the clear winner.
...
**Winner: DX Compact** — 12 tokens fewer than TOON (36% better).
...
**Verdict**: DX Compact **dominates** all tool-calling scenarios. The official TOON
encoder is significantly more verbose than hand-optimized TOON, especially on:
- Nested objects (TOON loses 9% vs DX's 20%)
- Schema definitions (TOON loses 27% vs DX's 56%)

The biggest win: **tool schema definitions** — DX saves **310 tokens per request**
vs JSON and **164 tokens** vs TOON. Over 100 requests, that's 31,000 tokens saved.

---

## Why DX Compact Wins Overall

| Advantage                    | DX Compact | TOON  | YAML  | JSON  |
|------------------------------|------------|-------|-------|-------|
| Token efficiency (general)   | **Best**   | 2nd   | 3rd   | 4th   |
| Tool call efficiency         | **Best**   | 4th   | 3rd   | 4th   |
| Schema definition efficiency | **Best**   | 3rd   | 2nd   | 4th   |
| Human readability            | Good       | Good  | Best  | Good  |
| Native Rust parser           | **Yes**    | JS    | C     | C     |
| Zero-copy deserialization    | **Yes**    | No    | No    | No    |
| Config format support        | **Yes**    | No    | Yes   | Yes   |
| Abbreviation engine          | **Yes**    | No    | No    | No    |
| LLM-optimized mode           | **Yes**    | No    | No    | No    |
| Round-trip fidelity          | **100%**   | 100%  | 100%  | 100%  |
...
├── small.toon-folded            # TOON with key folding
...
├── medium.toon-folded           # TOON with key folding
...
├── large.toon-folded            # TOON with key folding
