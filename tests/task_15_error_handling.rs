...
mod error_position_tests {
    ...
    fn test_unexpected_char_has_position() {
        ...
        if let Err(e) = result {
            if let ParseError::UnexpectedChar { pos, ch } = e {
                assert!(pos <= input.len(), "Position should be within input bounds");
    ...
    fn test_invalid_table_format_has_message() {
        ...
        if let Err(e) = result {
            if let ParseError::InvalidTable { msg } = e {
    ...
    fn test_multiline_error_position() {
        ...
        if let Err(e) = result {
            if let ParseError::UnclosedBracket { pos } = e {
    ...
    fn test_all_parse_errors_have_position_or_context() {
        ...
        for (input, description) in test_cases {
            ...
            assert!(
                result.is_err(),
                "Test case '{description}' should produce an error"
            );
            ...
            if let Err(e) = result {
                ...
                assert!(
                    has_useful_info,
                    "Error for '{description}' should provide useful information: {e:?}"
                );
            }
        }
    }
...
mod schema_mismatch_tests {
    ...
    fn test_schema_mismatch_with_nested_arrays() {
        ...
        if let Err(e) = result {
            if let ParseError::SchemaMismatch { expected, got: _ } = e {
    ...
    fn test_multiple_rows_first_row_mismatch() {
        ...
        if let Err(ParseError::SchemaMismatch { expected, got: _ }) = result {
            assert_eq!(expected, 2, "Should expect 2 columns");
    ...
    fn test_schema_mismatch_preserves_context() {
        ...
        if let Err(e) = result {
            if let ParseError::SchemaMismatch { expected, got } = e {
...
mod integration_tests {
    ...
    fn test_error_handling_comprehensive() {
        ...
        for (input, expected_error_type) in error_cases {
            ...
            assert!(
                result.is_err(),
                "Input '{input}' should produce {expected_error_type} error"
            );
