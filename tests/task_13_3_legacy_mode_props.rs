...
fn arb_array() -> impl Strategy<Value = Vec<DxLlmValue>> {
    prop::collection::vec(
        prop_oneof![
            ...
            "[a-zA-Z]{3,8}".prop_map(DxLlmValue::Str),
        ],
        2..6,
    )
}
...
fn arb_object() -> impl Strategy<Value = IndexMap<String, DxLlmValue>> {
...
fn arb_section() -> impl Strategy<Value = DxSection> {
    (2..5_usize, 2..5_usize).prop_flat_map(|(col_count, row_count)| {
        ...
        let rows = (0..row_count)
            .map(|i| {
                (0..col_count)
                    .map(|j| DxLlmValue::Str(format!("val{i}_{j}")))
                    .collect()
            })
            .collect();
...
fn extract_between(
    ...
    let start = text.find(start_delimiter)? + start_delimiter.len_utf8();
    let end = text[start..].find(end_delimiter)?;
...
proptest! {
    ...
    fn legacy_mode_objects_use_commas(obj in arb_object()) {
        ...
        if obj.len() > 1 {
            let object_content = extract_between(&output, '(', ')').unwrap_or("");
            prop_assert!(object_content.contains(','),
    ...
    fn legacy_mode_schemas_use_commas(section in arb_section()) {
        ...
        // Extract schema (between brackets)
        if let Some(schema) = extract_between(&output, '[', ']') {
