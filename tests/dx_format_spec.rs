...
mod zero_copy_format {
    ...
    fn spec_direct_memory_access() {
        ...
        unsafe {
            ...
            let id = *ptr.cast::<i32>();
            let value = *ptr.add(4).cast::<f64>();
            let flag = *ptr.add(12).cast::<u8>();
...
mod edge_cases {
    ...
    fn spec_empty_string() {
        let mut doc = DxDocument::new();
        doc.context
            .insert("empty".into(), DxLlmValue::Str(String::new()));
    ...
    fn spec_many_fields() {
        ...
        for i in 0..100 {
            doc.context
                .insert(format!("f{i}"), DxLlmValue::Num(f64::from(i)));
        }
...
mod performance {
    ...
    fn spec_llm_serialization_speed() {
        ...
        for i in 0..100 {
            doc.context
                .insert(format!("k{i}"), DxLlmValue::Num(f64::from(i)));
        }
        ...
        assert!(
            elapsed.as_secs() < 1,
            "LLM serialization too slow: {elapsed:?}"
        );
    }
    ...
    fn spec_machine_serialization_speed() {
        ...
        assert!(
            elapsed.as_secs() < 1,
            "Machine serialization too slow: {elapsed:?}"
        );
    }
