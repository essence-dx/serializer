...
fn simple_object() -> impl Strategy<Value = IndexMap<String, DxLlmValue>> {
...
fn simple_document() -> impl Strategy<Value = DxDocument> {
    (
        ...
        .prop_map(|(context, refs, sections)| {
            ...
            doc.context = context.into_iter().collect();
            doc.refs = refs.into_iter().collect();
...
fn objects_equivalent(a: &IndexMap<String, DxLlmValue>, b: &IndexMap<String, DxLlmValue>) -> bool {
    if a.len() != b.len() {
        return false;
    }
    for (key, val_a) in a {
