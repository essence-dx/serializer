...
mod parser_edge_cases {
    ...
    fn test_deeply_nested_keys() {
        ...
        let deep_key = (0..100)
            ...
            .collect::<Vec<_>>()
            .join(".");
        let input = format!("{deep_key}:value");
        let result = parse_str(&input);
...
mod mappings_edge_cases {
    ...
    fn test_mappings_unknown_keys_preserved() {
        ...
        for key in unknown_keys {
            ...
            assert_eq!(compressed, key, "Unknown key '{key}' should be preserved");

            let expanded = mappings.expand_key(key);
            assert_eq!(
                expanded, key,
                "Unknown key '{key}' should be preserved on expand"
            );
        }
    }
}
...
mod base62_edge_cases {
    ...
    fn test_base62_boundary_values() {
        ...
        for n in boundaries {
            ...
            let decoded = decode_base62(&encoded).unwrap();
            assert_eq!(decoded, n, "Boundary {n} should round-trip");
        }
    }
...
mod stress_tests {
    ...
    fn test_compression_large_input() {
        let mut input = String::new();
        for i in 0..1000 {
            input.push_str(&format!("context.item{i}: value{i}\n"));
        }
